...
class SMPEFilter(nn.Module):
    ...
    def __init__(self, filter_dim: int):
        super().__init__()
        self.filter_dim = filter_dim
        self.filter_params = nn.Parameter(torch.ones(filter_dim))

    def forward(self) -> torch.Tensor:
        # Constrained to [0, 1]
        return torch.sigmoid(self.filter_params)

# We don't really need the filter to be evolvable; commenting out for simplicity.
# class AMFilter(EvolvableModule):
#     def __init__(
#         self,
#         filter_dim: int,
#         device: str = "cpu",
...
#     ):
#         # This sets self.device, self.random_seed, and hooks into EvolvableModule
#         super().__init__(device=device, random_seed=random_seed)
#
#         self.filter_dim = filter_dim
#
#         # Actual parameters
...
class SMPE(MultiAgentRLAlgorithm):
    ...
    def __init__(
        ...
        self.filters = nn.ModuleDict( # for a non-evolvable version
                    {       agent_id: SMPEFilter(
                            filter_dim=self.others_obs_dims[agent_id] + self.others_act_dims[agent_id],
                            device=self.device)
                        for agent_id in self.agent_ids})
        ...
        for agent_id in self.agent_ids:
            ...
            #self.filters[agent_id] = filt.to(self.device)
        ...
        #self.filter_optimizers = OptimizerWrapper(
        #    optim.Adam, networks=self.filters, lr=lr_critic
        #)
        self.filter_optimizers = {
            agent_id: optim.Adam(self.filters[agent_id].parameters(), lr=lr_critic)
            for agent_id in self.agent_ids
        }
        ...
        #self.register_network_group(
        #    NetworkGroup(eval_network=self.filters)
        #)
    ...
    def _learn_agent(
        ...
        filter_opt = self.filter_optimizers[agent_id]
        ...
        own_obs = states[agent_id]
        own_next_obs = next_states[agent_id]
        own_actions = actions[agent_id].long().view(-1)
        r = rewards[agent_id].float().view(-1)
        d = dones[agent_id].float().view(-1)
        ...
        others_obs_list = [
            states[a] for a in self.agent_ids if a != agent_id
        ]
        others_next_obs_list = [
            next_states[a] for a in self.agent_ids if a != agent_id
        ]
        others_act_oh_list = []
        for other_id in self.agent_ids:
            ...
            other_act = actions[other_id].long().view(-1)
            others_act_oh_list.append(
                F.one_hot(other_act, num_classes=dim).float()
            )

        if len(others_obs_list) > 0:
            others_obs = torch.cat(others_obs_list, dim=-1)
            others_next_obs = torch.cat(others_next_obs_list, dim=-1)
            others_act = torch.cat(others_act_oh_list, dim=-1)
        else:
            others_obs = torch.zeros_like(own_obs)
            others_next_obs = torch.zeros_like(own_next_obs)
        ...
        # --- critic targets ---
        full_state = torch.cat([own_obs, others_obs], dim=-1)
        full_next_state = torch.cat([own_next_obs, others_next_obs], dim=-1)

        V_pred = critic_main(full_state).squeeze(-1)
        with torch.no_grad():
            V_next = critic_main(full_next_state).squeeze(-1)
        ...
        enc_input = torch.cat([own_obs, own_act_oh], dim=-1)
        ...
        if self.intrinsic_coef > 0.0:
            ...
            intrinsic_r = torch.tensor(
                intrinsic_r, device=self.device, dtype=torch.float32
            )
        else:
        ...
        target_return = total_r + self.gamma * (1.0 - d) * V_next

        value_loss_main = self.value_criterion(V_pred, target_return)

        # --- filtered critic + reconstruction + KL + filter reg ---
        # Decode others' info
        recon_target = torch.cat([others_obs, others_act], dim=-1)
        recon = decoder(belief)
        ...
        filt_others_obs = others_obs * phi_obs.unsqueeze(0)
        filt_state = torch.cat([own_obs, filt_others_obs], dim=-1)
        ...
        actor_input = torch.cat([own_obs, belief.detach()], dim=-1)
        logits = actor(actor_input)
        dist = torch.distributions.Categorical(logits=logits)
